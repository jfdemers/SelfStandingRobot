...
#define STEP1_PULSE GPIO_NUM_33
#define STEP1_DIR GPIO_NUM_32
#define STEP2_PULSE GPIO_NUM_26
#define STEP2_DIR GPIO_NUM_25
...
int16_t left_motor, throttle_left_motor, throttle_counter_left_motor, throttle_left_motor_memory = 0;
int16_t right_motor, throttle_right_motor, throttle_counter_right_motor, throttle_right_motor_memory = 0;
int16_t battery_voltage = 0;
int16_t receive_counter = 0;
int16_t gyro_pitch_data_raw, gyro_yaw_data_raw, accelerometer_data_raw = 0;

int32_t gyro_yaw_calibration_value, gyro_pitch_calibration_value = 0;

uint32_t loop_timer = 0;

float angle_gyro, angle_acc, angle, self_balance_pid_setpoint = 0;
float pid_error_temp, pid_i_mem, pid_setpoint, gyro_input, pid_output, pid_last_d_error = 0;
float pid_output_left, pid_output_right = 0;
...
void setup(){
  ...
  pinMode(STEP1_PULSE, OUTPUT);
  pinMode(STEP2_DIR, OUTPUT);
  pinMode(STEP2_PULSE, OUTPUT);
  ...
  for(receive_counter = 0; receive_counter < 500; receive_counter++){       //Create 500 loops
    ...
    gyro_yaw_calibration_value += (int16_t)(Wire.read()<<8|Wire.read());               //Combine the two bytes to make one integer
    gyro_pitch_calibration_value += (int16_t)(Wire.read()<<8|Wire.read());             //Combine the two bytes to make one integer
...
void loop(){
  ...
  accelerometer_data_raw = (int16_t)(Wire.read()<<8|Wire.read());                      //Combine the two bytes to make one integer
  ...
  gyro_yaw_data_raw = (int16_t)(Wire.read()<<8|Wire.read());                           //Combine the two bytes to make one integer
  gyro_pitch_data_raw = (int16_t)(Wire.read()<<8|Wire.read());                         //Combine the two bytes to make one integer
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Interrupt routine to drive stepper motors
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void IRAM_ATTR stepperTimer() {
  ...
  if(throttle_counter_right_motor > throttle_right_motor_memory){           //If the number of loops is larger then the throttle_right_motor_memory variable
    ...
    if(throttle_right_motor_memory < 0){                                    //If the throttle_right_motor_memory is negative
      digitalWrite(STEP1_DIR, LOW);                                                  //Set output 5 low to reverse the direction of the stepper controller
      throttle_right_motor_memory *= -1;                                    //Invert the throttle_right_motor_memory variable
    }
    else digitalWrite(STEP1_DIR, HIGH);                                               //Set output 5 high for a forward direction of the stepper motor
  }
  else if(throttle_counter_right_motor == 1) digitalWrite(STEP1_PULSE, HIGH);//Set output 4 high to create a pulse for the stepper controller
  else if(throttle_counter_right_motor == 2) digitalWrite(STEP1_PULSE, LOW);            //Set output 4 low because the pulse only has to last for 20us
